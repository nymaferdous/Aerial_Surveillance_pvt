...
class PyramidVisionTransformer(nn.Module):
    def __init__(self,img_size=224,loss='softmax', patch_size=16, in_chans=3, num_classes=1581, embed_dims=[64, 128, 256, 576],
        ...
        self.global_avgpool = nn.AdaptiveAvgPool2d(512)
        ...
        self.se = SELayer(400, 16)
        # cls_token
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dims[3]))

        # classification head

        self.head = nn.Linear(8, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        for blk in self.block4:
            x4 = blk(x4, H4, W4)
        x4 = x4.reshape(B, H4+1, W4+1, -1).permute(0, 3, 1, 2).contiguous()
        # x4 = self.norm(x4)
        return x1,x2,x3,x4
