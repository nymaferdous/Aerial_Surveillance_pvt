...
class PyramidVisionTransformer(nn.Module):
    def __init__(self,img_size=224,loss='softmax', patch_size=16, in_chans=3, num_classes=1581, embed_dims=[64, 128, 256, 576],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],use_gpu='True'):
        super().__init__()
        ...
        self.global_avgpool = nn.AdaptiveAvgPool2d(16)
        ...
        # classification head
        self.head1 = nn.Linear(128, num_classes) if num_classes > 0 else nn.Identity()
        self.head2 = nn.Linear(320, num_classes) if num_classes > 0 else nn.Identity()
    ...
    # def spatial_pyramid_pooling(self,im,levels):
    ...
    #         kh = h // i
    #         kw = w // i
    #         tensor = F.max_pool2d(im, kernel_size=(kh, kw),
    ...
    def spp(self,im):
        ...
        import torch.nn.functional as F
        # s, c, h, w = im.size()
        # x = F.max_pool2d(im, kernel_size=im.size()[2:])

        x = self.global_avgpool(im)
        # x = F.adaptive_avg_pool2d(im, (12))
        return x
        # s, c, h, w = im.size()
        # # print("{},{},{},{}".format(s,c,h,w))
        # pooling_layers = []
        ...
        # return torch.cat(pooling_layers, dim=-1)
    import math
    # def spatial_pyramid_pooling(self, previous_conv, num_sample, previous_conv_size, out_pool_size):
    #     '''
    ...
    def forward_features(self, x):
        ...
        x1 = self.pos_drop2(x1)
        for blk in self.block2:
            x1 = blk(x1, H1, W1)
        # x2=x
        x2 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()

        # stage 3
        x2, (H2, W2) = self.patch_embed3(x2)
        # print("Patch token x2", x2.shape)
        x2 = x2 + self.pos_embed3
        x2 = self.pos_drop3(x2)
        for blk in self.block3:
            x2 = blk(x2, H2, W2)
        # x3=x
        x3 = x2.reshape(B, H2, W2, -1).permute(0, 3, 1, 2).contiguous()
        # print("Patch token after reshape", x3.shape)

        # stage 4
        x3, (H3, W3) = self.patch_embed4(x3)
        # print("Patch token x4",x3.shape)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x3 = torch.cat((cls_tokens, x3), dim=1)
        x3 = x3 + self.pos_embed4
        x4 = self.pos_drop4(x3)
        for blk in self.block4:
            x4 = blk(x4, H3, W3)
        print("X4 shape",x4.shape)
        x4 = self.norm(x4)
        print("Normalize X4 shape", x4.shape)
    ...
    def forward(self, x):
        ...
        # x = self.spatial_pyramid_pooling(x,1,[int(x.size(2)),int(x.size(3))],levels)
        # x1=self.spp(x1)
        ...
        # print("After Flatten X4  Shape", x4.shape)
        # x5 = torch.flatten(x5)
        # x5 = self.spp(x5)
        # x5 = x5.view(x5.size(0), -1)
        # print("After Flatten X5  Shape", x5.shape)
        # x6=torch.cat((x4,x5),1)
        # x6 = x.unsqueeze(0)
        # print("After sqeeze", x.shape)
        # x6 = self.norm(x6)
        # x6 = x6.view(x6.size(0), -1)
        # x6 = self.norm(x6)
        # x=self.spatial_pyramid_pooling(x,levels)
        # print("Concat shape",x6.shape)
        x_1 = self.head1(x1)
        x_2 = self.head2(x2)
        x_3 = self.head3(x3)
        x_4 = self.head(x4)
...
@register_model
def pvt_medium(num_classes, loss='softmax',pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        num_classes=num_classes, loss=loss, patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1],use_gpu='True')
    # model.default_cfg = _cfg()
